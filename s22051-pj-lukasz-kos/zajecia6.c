...
Generalnie sygnał jest to przerwanie dostarczone do procesu. Systemy operacyjne używają sygnałów do raportowania wyjątkowych sytuacji 
np. błędów, eventów czy próśb. 
...
*/
int sigaction (
	/* Sygnał, który chcemy przechwycić. Lista sygnałów znajduje się poniżej
	https://man7.org/linux/man-pages/man7/signal.7.html */
	int signum, 

	/* struktura, w której zdefiniowaliśmy nowe procedury obsługi sygnału, maskę i flagi */
    const struct sigaction *restrict action, 
	
	/* zwraca DOMYŚLNĄ strukturę obsługi sygnału, ustawiamy NULL */
    strict sigaction *restrict old-action) 

/* 
Po tak zdefiniowanej strukturze sigaction oraz użytej funkcji sigaction, należy przede wszystkim pamiętać, 
że napisaną przez nas funkcję do obsługi sygnału, deklarujemy w strukturze sigaction przekazując wskaźnik do funkcji 
pod element sa_handler. 
...
Po stronie serwera.
Nazwa gniazda to adres. Nowo stworzone gniazdo (przy pomocy funkcji socket) nie ma adresu. 
Bez adresu inne procesy nie ustalą połączenia z gniazdem. 
Adres gniazda w internetowej przestrzenii nazw składa się z adresu internetowego urządzenia oraz numeru portu, 
który rozróżnia gniazda dla danego urządzenia. Zakres portów to 16 bit (0 - 65535). 
Standardowe serwery mają zarezerwowany zakres portów, szczegóły dalej.

Aby użyć typów danych niezbędnych do zdefiniowania formatu adresu gniazda w internetowej przestrzenii nazw 
należy dołączyć bibliotekę <netinet/in.h> i użyć struktury, opisujących adres w zależności od protokołu
*/
struct sockaddr_in {
...
Numery portów poniżej 1024 są zarezerwowane dla standardowych serverów takich jak finger i telnet i 
jedynie root ma do nich dostęp.
Numery portów równe i większe od 5000 to numery zarezerwowane dla serwerów i nie będą wykorzystywane 
...
Do tego służy funkcja listen(). 
Jeśli ustanowienie nasłuchu się nie powiedzie, funkcja zwróci wartość -1.
 
Następnie można zaakceptować nadchodzące połączenie funkcją accept. accept tworzy nowe gniazdo do komunikacji. 
...
/* funkcja zwraca deskryptor do nowego gniazda, które stanowi część połączenia */
